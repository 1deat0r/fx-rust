...
| Our size today | ~40K Rust LOC, 100 source files (incl. tests), 384 tests |
...
| Latest | 2026-08-22: **Phase 5 TUI screens landed** (`src/tui/screens.rs`: help / full-transcript / settings / resume / models / skills pickers + approval modal, ctrl-t/s/r/m/k shortcuts, OSC-11 theme auto-detection, activity spinner); **server fixes** (UTF-8-safe skill truncation, ACP session/new persistence); **Phase 7 SDK package landed** (`sdk/`: libfx package.json + README + 14 tests against `fxrs acp` + xterm adapter + examples) |
...
| CLI (interactive/ask/resume/sessions/session/status/permissions/setup/models/help/version/hooks/mcp/upgrade/doctor/usage/replay) | 🟡 | doctor/usage/replay landed (P1); auth + gh + terminal landed (P2/P3); **`pr`/`issue`/`credits`/`balance`/`provider` landed** (P4.5); review/capture/one-off/teams pending |
...
|---|---|---|
| Full TUI | `src/ui/**` (~150 files) | ✅ **`src/tui/`** — render engine, transcript runtime, composer, footer/help overlay (dock) + **dedicated screens** (help / full-transcript / settings / resume picker / models picker / skills catalog + approval modal) + OSC-11 theme auto-detection + activity spinner + resize. Remaining cosmetic depth: upstream pixel-level layout parity (full_transcript_screen is 8.3K Zig lines), notifications/sounds rendering |
...
| ACP (Agent Client Protocol) | `acp/*`, `core/cli/acp_runner.zig` | ✅ **`src/acp/` + `fxrs acp`** — newline-delimited JSON-RPC stdio server: `initialize` (protocol v1 + capabilities), `session/new` (**persists empty session**, configOptions/modes), `session/load`/`resume` (history replay notifications), `session/list`/`close`/`remove`, `session/prompt` (streams `session/update` agent_message_chunk/tool_call/tool_call_update via Human sink, returns stopReason; cooperative cancel via oneshot abort), `session/set_mode`, `session/set_config_option`, `session/cancel`; upstream ErrorCode table + event shapes |
| SDK + NAPI/WASM bindings | `napi_core_main.zig`, `wasm_core_main.zig`, `wasm_term_main.zig`, `sdk/**` | ✅ **`sdk/`** — dependency-free `libfx.mjs` (ACP-over-stdio `FxClient` + `prompt`), `package.json` (npm name libfx), README + API/event reference, **14 node:test integration tests** against `fxrs acp` (temp FX_HOME isolation), xterm adapter, `examples/basic.mjs`, term-demo, browser demo page. Differences (documented): upstream embeds WASM/NAPI cores; fxrs exposes the same agent surface through the ACP stdio server — **WASM/NAPI cores not yet ported** (future work) |
| Auth/login | `core/auth/*`, `core/hosts/native_keychain.zig` | 🟡 API-key store + `fxrs auth`/`login` landed (P2); OAuth flow + keychain pending |
| Gateway model catalog + credits | `core/gateway/*`, `builtins/gateway.zig` | ✅ **P2 — `src/gateway.rs`**: fetch + parse `GET {base}/coding-agent/v1/models` (public endpoint, anonymous fallback on 401/403), full `ModelCatalogEntry` capability metadata (tool-use/vision/reasoning/caching/context/max-tokens), upstream sort (tool-use → tier → provider → release), failure classification (auth/rate-limited/unavailable/transport/malformed), loopback-only base overrides, lazy cache + `context_limits_for`; wired into `fxrs models [--json|--offline|--limit N]` |
...
- **Phase 5 — TUI.** ✅ Render engine, transcript runtime, composer, footer/help dock, **all dedicated screens** (help/full-transcript/settings/resume/models/skills + approval modal), OSC-11 theme detection, resize, activity spinner (landed 172b25a). Remaining cosmetic depth: upstream pixel-level layout parity, notifications/sounds rendering.
- **Phase 6 — GitHub & advanced CLI.** git context, pr/issue publish, workflows; capture/one-off/gh/review/mcp_lookup commands.
- **Phase 7 — ACP + SDK/bindings.** ✅ ACP server/runner (`fxrs acp`); ✅ sdk/ JS bindings (libfx package, tests, xterm adapter, examples, term-demo). Remaining: NAPI + WASM cores (compile-to-wasm of the Rust core is future work; the ACP surface is the parity boundary today).
