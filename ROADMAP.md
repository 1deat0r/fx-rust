...
| Terminal integration | `core/terminal/*` (16 files), `tools/terminal/*`, `app_terminal*` | 🟡 **P3 — `src/terminal.rs` + `terminal` tool (tmux slice landed, native PTY in progress)**: per-session tmux socket, create/list/send/read (scrollback, ANSI strip, clear_after)/resize/stop, store + liveness reconcile, `fxrs terminal` + `/terminal`; **next-up: native PTY sessions (`backend` native|tmux, default native), `terminal exec`, `browser_terminal` tool, terminal-recovery decision model**; recovery/takeover (app_terminal takeover), browser runtime pending |
...
| terminal · browser_terminal | 🟡 (tmux sessions + native PTY; browser_terminal exec pending) |
...
- **Phase 3 — Execution & terminal.** ✅ background store + detached launch + `background_process` tool (start/list/get_output/log/supervise/tree/stop_tree) + session-tagged records + restore-on-resume banner + `fxrs background` + `/background`. ✅ tmux terminal sessions (`terminal` tool create/list/send/read/resize/stop, `fxrs terminal` + `/terminal`, tmux-liveness reconcile; `fxrs doctor` checks tmux/background/terminal stores). Remaining: native PTY + browser terminal, recovery/takeover, devbox executors (operator priority: user-visible terminal surfaces first, then recovery, then infra executors).
