...
| Our size today | ~23.1K Rust LOC, 55 source files, 212 tests |
...
| Full subagent system | `core/subagent/*` (21 files) + `ui/subagent/*` | 🟡 **P4 — `src/subagent_domain.rs` landed (52f7ae1)**: full type + validation layer of domain.zig (bounds, State/Mode/QueueStatus, Create/Inspect/Message/Relationship/Configure/Lifecycle commands, NotificationPolicy validation, id/model/op-id/cursor validation, inspectWaitSatisfied), wired into the subagent tool (create validated through validate_command; name + permission_mode args). Remaining: authority.zig, execution/manager, approvals, communication, tool host, ui projection |
...
- **Phase 4 — Subagent & modes.** Modes/mods registries landed (efbb450); subagent domain model + tool validation wiring landed (52f7ae1). Remaining: subagent authority/execution/manager/communication/tool-host + UI, then modes UI.
