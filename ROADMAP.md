...
- **Phase 3 — Execution & terminal.** ✅ background store + detached launch + `background_process` tool (start/list/get_output/log/stop) + `fxrs background` + `/background`. Remaining: process supervisor/restore-on-resume, process tree, local + devbox executors, terminal integration (native/tmux/browser), `terminal` tooling.
